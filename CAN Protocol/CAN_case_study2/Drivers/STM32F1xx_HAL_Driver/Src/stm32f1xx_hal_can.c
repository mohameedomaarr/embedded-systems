...
void HAL_CAN_IRQHandler(CAN_HandleTypeDef *hcan)
{
  ...
  {
    /* Transmit Mailbox 0 management  (indicating that Transmission Mailbox 0 has completed)*****************************************/
    if ((tsrflags & CAN_TSR_RQCP0) != 0U)
    {
      /* Clear the Transmission Complete flag (and TXOK0,ALST0,TERR0 bits) */
      __HAL_CAN_CLEAR_FLAG(hcan, CAN_FLAG_RQCP0);
      /* checks whether the transmit OK flag for FIFO 0 is set in the CAN transmit status register
       * This flag indicates that a message has been successfully transmitted from FIFO 0 */
